...
TitleScene.prototype.create = function(){
  this.add.image(W/2,H/2,'bg1').setAlpha(0.95);
  this.add.rectangle(W/2,H/2,720,360,0x000000,0.35).setStrokeStyle(3,0xffffff,0.35);
  this.add.text(W/2,70,'Heart Quest: Wake Jenny',{fontFamily:'Arial',fontSize:'40px',color:'#fff',stroke:'#000',strokeThickness:6}).setOrigin(0.5);
  ...
  this.add.text(W/2,H/2,intro,{fontFamily:'Arial',fontSize:'22px',color:'#fff',align:'center',lineSpacing:8}).setOrigin(0.5);
  this.add.text(W/2,H-70,'Tryck för att börja',{fontFamily:'Arial',fontSize:'22px',color:'#fff'}).setOrigin(0.5);
  const z=this.add.image(W-90,90,'snore').setScale(2).setAlpha(0.7);
  this.tweens.add({targets:z,y:70,duration:1200,yoyo:true,repeat:-1,ease:'Sine.easeInOut'});
...
Level1Scene.prototype.create = function(){
  ...
  this.player=this.physics.add.sprite(80,H-120,'fredrik').setScale(1.4);
  ...
  spots.forEach(([x,y])=>{
    const h=this.hearts.create(x,y,'heart').setScale(1.8);
  ...
  this.door=this.physics.add.staticImage(2100,H-56,'door').setOrigin(0.5,1).setScale(2);
  ...
  this.hudHearts=this.add.text(16,16,`❤️ 0/${REQUIRED_HEARTS}`,{fontFamily:'Arial',fontSize:'24px',color:'#fff',stroke:'#000',strokeThickness:6}).setScrollFactor(0);
  this.toast=this.add.text(W/2,60,'',{fontFamily:'Arial',fontSize:'22px',color:'#fff',stroke:'#000',strokeThickness:6}).setOrigin(0.5).setScrollFactor(0).setAlpha(0);
...
Level1Scene.prototype.createTouchControls=function(){
  ...
  const makeBtn=(x,label)=>{
    const bg=this.add.rectangle(x,padY,90,70,0x000000,0.25).setStrokeStyle(3,0xffffff,0.35).setScrollFactor(0);
    const t=this.add.text(x,padY,label,{fontFamily:'Arial',fontSize:'28px',color:'#fff'}).setOrigin(0.5).setScrollFactor(0);
    bg.setInteractive();
    return bg;
  };
  const left=makeBtn(80,'◀'), right=makeBtn(180,'▶'), jump=makeBtn(W-100,'⤒');
...
Level1Scene.prototype.showToast=function(msg){
  ...
  this.tweens.add({targets:this.toast,alpha:0,duration:800,delay:800});
};

Level1Scene.prototype.update=function(){
  const speed=260, jumpSpeed=520;
  ...
  if(this.touch.jump) this.touch.jump=false; // tap-to-jump
};
...
WinScene.prototype.create=function(){
  this.add.image(W/2,H/2,'bg1').setAlpha(0.95);
  this.add.rectangle(W/2,H/2,720,320,0x000000,0.35).setStrokeStyle(3,0xffffff,0.35);
  this.add.text(W/2,120,'DEMO KLAR!',{fontFamily:'Arial',fontSize:'44px',color:'#fff',stroke:'#000',strokeThickness:8}).setOrigin(0.5);
  const msg=[
    `Du samlade ${this.dataFromLevel.hearts||0} hjärtan ❤️`,'',
    'Nästa steg: 5 banor + snark-monster-Jenny,',
    'Fredrik-jumpscares & finalscenen (C).'
  ].join('\n');
  this.add.text(W/2,H/2+40,msg,{fontFamily:'Arial',fontSize:'22px',color:'#fff',align:'center',lineSpacing:8}).setOrigin(0.5);
